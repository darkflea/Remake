...
-- Load the collection of core scripts, required for everything else to work
	...
	function remake.showcategories()
		local categories = {}

		for option in premake.option.each() do
			local cat = option.category and ("OPTIONS - " .. option.category) or "OPTIONS - General"
		...
		for k, options in spairs(categories) do
			...
			for _, option in ipairs(options) do
				...
				length = math.max(length, #trigger)
			end
			...
			for _, option in ipairs(options) do
				...
				if option.allowed then
					table.sort(option.allowed, function(a, b)
	...
	function remake.showhelp()

		-- display the basic usage
		printf("Remake %s, a build script generator", _REMAKE_VERSION)
		printf("Based on Premake - %s %s", _VERSION, _PREMAKE_COPYRIGHT)
		printf("Lua %s",  _COPYRIGHT)
		printf("")
		printf("Usage: premake5 [options] action [arguments]")
		printf("")

		remake.showcategories()
		remake.showactions()
...
---
	...
	function m.locateUserScript()
		local defaults = { "premake5.lua", "premake4.lua", "remake1.lua" }
		for i = 1, #defaults do
...
--
	...
	function _premake_premain()
		p.callArray(m._premain)
	end

	function _premake_main()
		p.callArray(m._premakemain)
	end

	function _premake_postmain()
		p.callArray(m._postmain)
		return 0
	end