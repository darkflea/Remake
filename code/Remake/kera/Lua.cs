...
#pragma warning restore CA1063 // Implement IDisposable Correctly
        ...
        /// Dumps a function as a binary chunk. Receives a Lua function on the top of the stack and produces a binary chunk that, if loaded again, results in a function equivalent to the one dumped
        /// </summary>
        /// <param name="stripDebugInfo"></param>
        /// <returns></returns>
        public byte[] Dump(bool stripDebugInfo = false)
        {
            using var ms = new MemoryStream();

            LuaWriter writer = (IntPtr L, IntPtr p, UIntPtr size, IntPtr ud) =>
            {
                int len = (int)size;
            ...
            // Call your existing low-level API
            this.Dump(writer, IntPtr.Zero, stripDebugInfo);
        ...
        {
            //LogManager.Instance.LogDeveloperMessage($"Getting global '{name}'");
        ...
        {
            //LogManager.Instance.LogDeveloperMessage($"Getting integer field '{i}' from index {index}");
        ...
        public LuaStatus Load(byte[] chunk, string chunkName, string mode)
        {
            int index = 0;

            LuaReader reader = (IntPtr L, IntPtr data, ref nuint size) =>
            {
                ...
                int remaining = chunk.Length - index;
                size = (nuint)remaining;

                IntPtr ptr = Marshal.AllocHGlobal(remaining);
                Marshal.Copy(chunk, index, ptr, remaining);
            ...
            return Load(reader, IntPtr.Zero, chunkName, mode);
        }
...
#pragma warning restore IDE0018 // Inline variable declaration
        ...
        /// <summary>
        /// Pushes a copy of the value at the specified stack index onto the top of the Lua stack.
        /// </summary>
        /// <param name="index">The stack index of the value to copy.</param>
