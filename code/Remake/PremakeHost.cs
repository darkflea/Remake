...
namespace Remake
{
    public class PremakeHost
    {
        ...
        private static readonly LuaFunction SearcherLuaDelegate = PremakeFinder.PremakeSearcherLua;
        ...
        public PremakeResult Init()
        {
            ...
            l.PushCClosure(SearcherLuaDelegate, 1); // [package, searchers, 2, closure]
            ...
            l.PushString(Program.PremakeRootDirectory);
        ...
        public PremakeResult Execute(string[] args, string script)
        {
            ...
            try
            {
                Console.WriteLine("Running _premake_premain...");
                ...
                Console.WriteLine("Running _premake_main...");
                result = RunStage(l, "_premake_main");

                if (isHelp && helpSystem != null)
                {
                ...
                {
                    Console.WriteLine("No help requested, skipping help display.");
                    Console.WriteLine("Running _premake_postmain...");
        ...
        private LuaStatus RunPremakeMain(Lua L, string script)
        {
            ...
            /* Release builds want to load embedded scripts, with --scripts
             * arg allowed as override. Debug builds will look at local
             * file system first, then fall back to embeded. */
#if !DEBUG
             z = PremakeFinder.PremakeLocateFile(L, script, FileSearchMask.SearchScripts | FileSearchMask.SearchEmbedded);
#else
            z = PremakeFinder.PremakeLocateFile(L, script,
                FileSearchMask.SearchLocal | FileSearchMask.SearchScripts |
                FileSearchMask.SearchPath | FileSearchMask.SearchEmbedded);
#endif

            /* If no embeded script found, release builds will then
             * try to fall back to local file system, just in case */
#if !DEBUG
             if (z != LuaStatus.OK)
             {
                 z = PremakeFinder.PremakeLocateFile(L, script, FileSearchMask.SearchLocal | FileSearchMask.SearchPath);
             }
#endif

            if (z == LuaStatus.OK)
            {
                // Path is at the top of the stack after a successful PremakeLocateFile
                string filename = L.ToString(-1);
                z = OverrideFunctions.PremakeLuaLDoFile(L, filename);
            }
