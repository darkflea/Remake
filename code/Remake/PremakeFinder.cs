...
namespace Remake
{
    ...
    public static class PremakeFinder
    {
        ...
        public static LuaStatus PremakeLocateFile(Lua l, string filename, FileSearchMask searchMask)
        {
            ...
            {
                // Use your previously defined FindEmbeddedScript helper
                var mapping = PremakeLoader.FindEmbeddedScript(filename);
                if (mapping != null)
                {
        ...
        public static int PremakeSearcherLua(IntPtr state)
        {
            ...
            if (PremakeLoader.FindEmbeddedScript(embeddedPath) != null)
            {
                LuaStatus status = PremakeLoader.PremakeLoadEmbeddedScript(l, embeddedPath);
                if (status == LuaStatus.OK)
                {
